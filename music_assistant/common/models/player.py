...
class PlayerGroup(Player):
    ...
    _attr_supported_features: Tuple[PlayerFeature] = (
        PlayerFeature.POWER,
        PlayerFeature.MUTE,
    )
    ...
    @property
    def volume_muted(self) -> int:
        """Calculate a group volume mute state from the grouped members."""
        # returns true if all childs are muted
        for child_player in self.get_child_players():
            if not child_player.volume_muted:
                return False
        return True
    ...
    async def mute(self, powered: bool) -> None:
        """Send POWER command to (group) player."""
        await super().power(powered)
        # turn on/off group childs
        coros = [
