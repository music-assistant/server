"""Pocket Casts Music Provider implementation."""
...
class PocketCastsProvider(MusicProvider):
    ...
    def __init__(
        ...
        """Initialize the Pocket Casts provider."""
    ...
    async def _get_auth_token(self) -> str | None:
        ...
        :return: Bearer token or None if login fails.
        """
    ...
    async def _get_headers(self) -> dict[str, str]:
        ...
        :return: Dict of HTTP headers including Authorization.
        :raises LoginFailed: If not authenticated.
        """
        token = await self._get_auth_token()
        if not token:
    ...
    async def get_library_podcasts(self) -> AsyncGenerator[Podcast, None]:
        ...
        for podcast_data in podcasts:
            try:
                podcast = parse_podcast(self, podcast_data)
                yield podcast
            except Exception as err:
                self.logger.warning(
    ...
    async def get_podcast(self, prov_podcast_id: str) -> Podcast:
        ...
        :param prov_podcast_id: The podcast UUID.
        :return: Podcast object.
        :raises MediaNotFoundError: If podcast not found.
        """
        ...
        for podcast_data in podcasts:
            if podcast_data.get("uuid") == prov_podcast_id:
                return parse_podcast(self, podcast_data)
    ...
    async def _get_bookmarked_episodes(self) -> list[PodcastEpisode]:
        ...
        for bookmark in data.get("bookmarks", []):
            try:
                episode = parse_bookmark(self, bookmark)
                episodes.append(episode)
            except Exception as err:
                self.logger.warning(
    ...
    async def get_podcast_episode(self, prov_episode_id: str) -> PodcastEpisode:
        ...
        :raises MediaNotFoundError: If episode not found.
        """
    ...
    async def get_stream_details(
        ...
        :return: StreamDetails for the episode.
        :raises MediaNotFoundError: If episode or stream URL not found.
        """
    ...
    async def get_resume_position(self, item_id: str, media_type: MediaType) -> tuple[bool, int]:
        ...
        :raises NotImplementedError: If media_type is not PODCAST_EPISODE or no progress info.
        :raises MediaNotFoundError: If episode ID format is invalid.
        """
