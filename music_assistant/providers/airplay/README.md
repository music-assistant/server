...
**Note**: The projection can only push a joiner's anchor later, never earlier — `AIRPLAY_LATE_JOIN_MIN_HEADROOM_MS` is the floor, and the whole anchor when no projection arrives. The binary also runs a post-commit clock verification that can pull an anchor forward, but it only arms when the receiver has still not probed by the time it reads the START, and only for an anchor that clears the receiver queue depth plus 500 ms. Past ~2 s of effective depth (see `AIRPLAY_BUFFER_DEPTH_DEFAULTS` in [constants.py](constants.py)) a joiner's anchor no longer clears that, by design: the queue starts releasing frames one depth *before* the anchor, and a line with audio on the wire cannot move.
