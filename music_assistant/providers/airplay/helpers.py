...
async def resolve_if_ip(mass: MusicAssistant, target_ip: str) -> str:
    ...
    """
    # Prefer an explicitly configured zeroconf interface. The setting may be a
    ...
    for candidate in zc_iface.split(","):
        ...
        if iface and iface not in ("default", "all"):
            return iface
    # Otherwise resolve a locally-bindable interface IP reachable by the target:
    # bind_ip if concrete, else a per-device routing-table lookup, else publish_ip.
    return await mass.streams.resolve_local_ip(target_ip)
