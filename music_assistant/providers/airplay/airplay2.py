...
class AirPlay2StreamSession:
    ...
    async def start(self) -> None:
        """Initialize AirPlay2StreamSession."""
        # initialize airplay2 stream for all players

        # get current ntp and start AirPlay2Stream per player
        assert self.prov.cliairplay2_bin
        _, stdout = await check_output(self.prov.cliairplay2_bin, "-ntp")
        ...
        async def _start_client(airplay2_player: AirPlayPlayer) -> None:
            ...
            if airplay2_player.airplay2_stream and airplay2_player.airplay2_stream.running:
                await airplay2_player.airplay2_stream.stop()

            airplay2_player.airplay2_stream = AirPlay2Stream(self, airplay2_player)
            await airplay2_player.airplay2_stream.start(start_ntp, wait_start)
    ...
    async def remove_client(self, airplay_player: AirPlayPlayer) -> None:
        ...
        if airplay_player not in self.sync_clients:
            return
        assert airplay_player.airplay2_stream
        assert airplay_player.airplay2_stream.session == self
        async with self._lock:
            self.sync_clients.remove(airplay_player)
        await airplay_player.airplay2_stream.stop()
        airplay_player.airplay2_stream = None
    ...
    async def replace_stream(self, audio_source: AsyncGenerator[bytes, None]) -> None:
        ...
        for sync_client in self.sync_clients:
            if not sync_client.airplay2_stream:
                continue  # guard
            sync_client.airplay2_stream.start_ffmpeg_stream()
            ...
            async for chunk in self._audio_source:
                async with self._lock:
                    sync_clients = [
                        ...
                        if x.airplay2_stream and x.airplay2_stream.running
                    ]
                    if not sync_clients:
                        return
                    await asyncio.gather(
                        *[
                            x.airplay2_stream.write_chunk(chunk)
                            for x in sync_clients
                            if x.airplay2_stream
                        ],
                        return_exceptions=True,
                    )
...
class AirPlay2Stream:
    ...
    of AirPlay2 audio using a small executable written in C based on Owntones AirPlay2 code to do
    ...
    def __init__(
        ...
        self._cliairplay2_proc: AsyncProcess | None = None
    ...
    def running(self) -> bool:
        ...
        return (
            ...
            and self._cliairplay2_proc is not None
            and not self._cliairplay2_proc.closed
        )

    async def start(self, start_ntp: int, wait_start: int = 1000) -> None:
        """Initialize cliairplay2 process for a player."""
        assert self.prov.cliairplay2_bin
        extra_args: list[str] = []
        ...
        # audio to the cliairplay2 process
        self.start_ffmpeg_stream()

        # cliairplay2 is the binary that handles the actual AirPlay2 streaming to the player
        # this has been adapted from the cliraop binary used in MASS and the AirPlay2 support
        # implemented in OwnTones
        # https://github.com/music-assistant/libairplay2
        # we use this intermediate binary to do the actual streaming because attempts to do
        # so using pure python (e.g. pyatv) for raop were not successful due to the realtime nature
        cliairplay2_args = [
            self.prov.cliairplay2_bin,
            "-ntpstart",
            str(start_ntp),
            "-port",
        ...
        self._cliairplay2_proc = AsyncProcess(
            cliairplay2_args, stdin=True, stderr=True, name="cliairplay2"
        )
        if platform.system() == "Darwin":
            os.environ["DYLD_LIBRARY_PATH"] = "/usr/local/lib"
        await self._cliairplay2_proc.start()
        # read first 20 lines of stderr to get the initial status
        for _ in range(20):
            line = (await self._cliairplay2_proc.read_stderr()).decode("utf-8", errors="ignore")
        ...
        # start reading the stderr of the cliairplay2 process from another task
    ...
    async def stop(self) -> None:
        ...
        if self._ffmpeg_reader_task and not self._ffmpeg_reader_task.done():
            self._ffmpeg_reader_task.cancel()
        if self._cliairplay2_proc and not self._cliairplay2_proc.closed:
            await self._cliairplay2_proc.close(True)
    ...
    async def send_cli_command(self, command: str) -> None:
        """Send an interactive command to the running cliairplay2 binary."""
        if self._stopped or not self._cliairplay2_proc or self._cliairplay2_proc.closed:
            return
        await self._started.wait()
        ...
        named_pipe = f"/tmp/airplay2-{self.active_remote_id}"  # noqa: S108
    ...
    def start_ffmpeg_stream(self) -> None:
        """Start (or replace) the player-specific ffmpeg stream to feed cliairplay2."""
    ...
    async def _ffmpeg_reader(self) -> None:
        """Read audio from the audio source and pipe it to the cliairplay2 process."""
        ...
        # wait for cliairplay2 to be ready
        await asyncio.wait_for(self._started.wait(), 20)
        async for chunk in self._ffmpeg_proc.iter_chunked(chunksize):
            if self._stopped:
                break
            if not self._cliairplay2_proc or self._cliairplay2_proc.closed:
                break
            await self._cliairplay2_proc.write(chunk)
        ...
        if self._cliairplay2_proc and not self._cliairplay2_proc.closed:
            await self._cliairplay2_proc.write_eof()

    async def _stderr_reader(self) -> None:
        """Monitor stderr for the running cliairplay2 process."""
        ...
        if not self._cliairplay2_proc:
            return
        async for line in self._cliairplay2_proc.iter_stderr():
