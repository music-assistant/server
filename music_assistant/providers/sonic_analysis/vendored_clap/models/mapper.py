    ...
    def __init__(
        ...
        for i in range(num_layers):
            if i % 2 == 0 and enc_dec:  # cross
                layers.append(
                    TransformerLayer(
                        dim_self, dim_ref, num_heads, mlp_ratio, act=act, norm_layer=norm_layer
                    )
                )
            elif enc_dec:  # self
                layers.append(
                    TransformerLayer(
                        dim_self, dim_self, num_heads, mlp_ratio, act=act, norm_layer=norm_layer
                    )
                )
            else:  # self or cross
                layers.append(
...
class ClapCaptionModel(nn.Module):
    def __init__(
        ...
        freeze_audio_encoder_weights: bool = True,
        ...
        if mapping_type == "mlp":
            ...
            self.clap_project = TransformerMapper(
    ...
    def forward(
        ...
        prefix_projections = self.clap_project(prefix).view(
            -1, self.prefix_length, self.gpt_embedding_size
        )
        embedding_cat = torch.cat((prefix_projections, embedding_text), dim=1)
        if labels is not None:
            dummy_token = self.get_dummy_token(
                tokens["input_ids"].shape[0], tokens["input_ids"].device
            )
