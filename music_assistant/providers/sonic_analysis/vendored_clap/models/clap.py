...
class TextEncoder(nn.Module):
    # MA MOD: skip_text_model lets the audio-only path skip the GPT2 (or other
    # HF text model) download/load entirely — saves ~500MB when the provider
    # is configured without text search.
    def __init__(
        self,
        d_out: int,
        text_model: str,
...
class CLAP(nn.Module):
    # MA MOD: skip_text_encoder forwarded into TextEncoder so audio-only
    # callers can avoid the HF text-model download.
    def __init__(
        self,
        # audio
        audioenc_name: str,
        ...
        *,
        ...
        self.caption_encoder = TextEncoder(
            ...
            skip_text_model=skip_text_encoder,
        )
