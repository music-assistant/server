...
class HTSAT_Swin_Transformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        if self.config.enable_tscam:
            ...
            if self.config.htsat_attn_heatmap:
                fpx = interpolate(
                    torch.sigmoid(x).permute(0, 2, 1).contiguous() * attn, 8 * self.patch_stride[1]
                )
            else:
                fpx = interpolate(
    ...
    def reshape_wav2img(self, x):
        ...
        x = x.reshape(
            x.shape[0], x.shape[1], x.shape[2], self.freq_ratio, x.shape[3] // self.freq_ratio
        )
        # print(x.shape)
    ...
    def forward(
        ...
        if infer_mode:
            ...
            if self.training:
                ...
                clipwise_output = (
                    torch.zeros_like(output_dicts[0]["clipwise_output"]).float().to(x.device)
                )
                framewise_output = (
                    torch.zeros_like(output_dicts[0]["framewise_output"]).float().to(x.device)
                )
            ...
            if x.shape[2] > self.freq_ratio * self.spec_size:
                if self.training:
                    ...
                    latent_output = (
                        torch.zeros_like(output_dicts[0]["latent_output"]).float().to(x.device)
                    )
                    for d in output_dicts:
                        clipwise_output += d["clipwise_output"]
                        framewise_output += d["framewise_output"]
