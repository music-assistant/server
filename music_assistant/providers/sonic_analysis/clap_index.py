"""CLAP audio-embedding ANN index for text-to-track library search.
...
Storage lives under mass.storage_path as two sibling files:
  sonic_analysis_clap_v1.usearch    — HNSW graph + f16 vectors
  sonic_analysis_clap_v1_keys.json  — label → [provider, item_id]

Both are debounced-flushed: at least SAVE_MIN_ADDS adds AND at least
SAVE_MIN_INTERVAL_SECONDS since the last save, plus an unconditional
flush on close. Between saves the state lives only in memory; a crash
loses unflushed adds, which the background scan naturally re-picks via
contains() checks on the next pass.
"""
...
class ClapIndex:
    ...
    def __init__(
        ...
        filename_stem: str = "sonic_analysis_clap_v1",
    ) -> None:
