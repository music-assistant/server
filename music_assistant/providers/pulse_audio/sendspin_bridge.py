...
class LocalPulseAudioBridgeManager:
    ...
    async def discover_and_register(self) -> None:
        ...
        loop = asyncio.get_running_loop()
        try:
            sinks: list[dict[str, Any]] = await loop.run_in_executor(None, self._enumerate_pa_sinks)
        ...
        async with self._lock:
            for sink in sinks:
                ...
                bridge = SendspinPulseAudioBridge(self.provider, player, sink, sendspin_server)
                try:
                    await bridge.start()
                except Exception:
