"""Parsing utilities for Spotify provider."""
...
def parse_images(
    ...
    for img in images_list:
        img_url = img["url"]
        # Skip generic placeholder images for artists
...
def parse_artist(artist_obj: dict[str, Any], provider: SpotifyProvider) -> Artist:
    ...
    artist.metadata.images = UniqueList(
        parse_images(artist_obj.get("images", []), provider.lookup_key, exclude_generic=True)
    )
    return artist
...
def parse_album(album_obj: dict[str, Any], provider: SpotifyProvider) -> Album:
    ...
    if upc := external_ids.get("upc"):
        album.external_ids.add((ExternalID.BARCODE, "0" + upc))
    if ean := external_ids.get("ean"):
        album.external_ids.add((ExternalID.BARCODE, ean))
...
def parse_track(
    track_obj: dict[str, Any], provider: SpotifyProvider, artist: Artist | None = None
) -> Track:
    ...
    for track_artist in track_obj.get("artists", []):
        if track_artist.get("name") and track_artist.get("id"):
            artist = parse_artist(track_artist, provider)
...
def parse_podcast_episode(
    ...
    """Parse spotify podcast episode object to generic layout with played status sync."""
    ...
    # SYNC PLAYED STATUS FROM SPOTIFY TO MA
    if provider.sync_played_status_enabled and "resume_point" in episode_obj:
        resume_point = episode_obj["resume_point"]
        sync_episode_status_to_ma(episode, resume_point, provider)

    return episode


def sync_episode_status_to_ma(
    episode: PodcastEpisode, resume_point: dict[str, Any], provider: SpotifyProvider
) -> None:
    """
    Sync played status and resume position from Spotify to Music Assistant episode object.

    This sets the fields that MA will use when the episode is yielded.
    """
    if not resume_point:
        return
    ...
    # Set MA episode fields directly
    episode.resume_position_ms = resume_position_ms
    episode.fully_played = fully_played

    # Determine if episode should be marked as played based on threshold
    should_mark_played = fully_played

    if not fully_played and episode.duration > 0 and resume_position_ms > 0:
        completion_ratio = resume_position_ms / (episode.duration * 1000)
        if completion_ratio >= provider.played_threshold:
            should_mark_played = True
            episode.fully_played = True  # Override for MA
            provider.logger.debug(
                f"Episode {episode.item_id} is {completion_ratio:.1%} complete "
                f"(threshold: {provider.played_threshold:.1%}) - marking as played"
            )
