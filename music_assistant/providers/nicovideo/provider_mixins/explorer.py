...
class NicovideoMusicProviderExplorerMixin(NicovideoMusicProviderMixinBase):
    ...
    async def search(
        ...
        if MediaType.TRACK in media_types:
            tracks = await self.service_hub.search.search_videos_by_keyword(search_query, limit)
        ...
        if list_media_types:
            await self.service_hub.search.search_playlists_and_albums_by_keyword(
    ...
    async def recommendations(self) -> list[RecommendationFolder]:
        ...
        history_tracks = await self.service_hub.user.get_user_history(limit=history_count)
        if history_tracks:
            recommendation_folders.append(
                RecommendationFolder(
        ...
        following_tracks = await self.service_hub.user.get_following_activities(
        ...
        like_history_tracks = await self.service_hub.user.get_like_history(limit=like_history_count)
        ...
        if recommendation_tags:
            for tag in recommendation_tags:
                tag_recommendation_tracks = await self.service_hub.search.search_videos_by_tags(
        ...
        if new_tracks_tags:
            for tag in new_tracks_tags:
                new_tracks_by_tags = await self.service_hub.search.search_videos_by_tags(
    ...
    async def _fetch_recommendations_with_filtering(self, target_count: int) -> list[Track]:
        ...
        if not required_tags:
            ...
            return await self.service_hub.user.get_recommendations(limit=target_count)
        ...
        for attempt in range(max_attempts):
            ...
            try:
                batch_tracks = await self.service_hub.user.get_recommendations(
    ...
    async def _fetch_similar_tracks_with_filtering(
        ...
        if not required_tags:
            # No filtering needed, just fetch the target count
            return await self.service_hub.user.get_similar_tracks(prov_track_id, limit=target_count)
        ...
        try:
            tracks = await self.service_hub.user.get_similar_tracks(
