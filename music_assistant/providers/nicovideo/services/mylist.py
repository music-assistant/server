...
class NicovideoMylistService(NicovideoBaseService):
    ...
    async def get_own_mylists(self) -> list[Playlist]:
        ...
        results = await self.service_manager._call_with_throttler(
            self.service_manager.niconico_py_client.user.get_own_mylists
        )
        if results is None:
            return []
    ...
    async def get_mylist(
        ...
        mylist = await self.service_manager._call_with_throttler(
            self.service_manager.niconico_py_client.video.get_mylist,
            mylist_id,
            page_size=page_size,
            page=page,
        )
        if not mylist:
            return None
        return self.converter_manager.playlist.convert_with_tracks_by_mylist(mylist)
    ...
    async def get_own_mylist(
        ...
        mylist = await self.service_manager._call_with_throttler(
            self.service_manager.niconico_py_client.user.get_own_mylist,
            mylist_id,
            page_size=page_size,
            page=page,
        )
    ...
    async def add_mylist_item(self, mylist_id: str, video_id: str) -> bool:
        ...
        result = await self.service_manager._call_with_throttler(
            self.service_manager.niconico_py_client.user.add_mylist_item,
            mylist_id,
            video_id,
        )
        return bool(result)
    ...
    async def remove_mylist_items(self, mylist_id: str, video_ids: list[str]) -> bool:
        ...
        result = await self.service_manager._call_with_throttler(
            self.service_manager.niconico_py_client.user.remove_mylist_items,
            mylist_id,
            video_ids,
        )
        return bool(result)
    ...
    async def create_mylist(
        ...
        return await self.service_manager._call_with_throttler(
            self.service_manager.niconico_py_client.user.create_mylist,
            name,
