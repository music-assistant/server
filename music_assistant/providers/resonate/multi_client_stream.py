...
class MultiClientStream:
    ...
    def position_us(self) -> int:
        """Return current position in microseconds based on bytes streamed."""
        # Calculate microseconds from bytes using audio format
        # microseconds = total_bytes * 8_000_000 / (bit_depth * channels * sample_rate)
        bytes_per_second = (
        ...
        if bytes_per_second == 0:
            return 0
        return int((self.position_bytes * 1_000_000) / bytes_per_second)
        ...
        async for chunk in self.audio_source:
            ...
            while len(self.subscribers) == 0:
                ...
                if fail_count > 50:
                    ...
                    return
            # Track position in bytes
            self.position_bytes += len(chunk)
