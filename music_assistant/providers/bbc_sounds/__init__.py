...
class BBCSoundsProvider(RecommendationPayloadMixin, MusicProvider):
    ...
    async def get_radio(self, prov_radio_id: str) -> Radio:
        ...
        self.logger.debug(
            "%s %s %s.%s", station, ma_radio, type(ma_radio).__module__, type(ma_radio).__name__
        )
            ...
            if media_type != MediaType.RADIO:
                ...
                if action:
                    try:
                        ...
                        self.logger.error("Error updating play status: %s", err)
    ...
    def _stream_error(self, item_id: str, media_type: MediaType) -> MusicAssistantError:
        return MusicAssistantError("Couldn't get stream details for %s (%s)", item_id, media_type)
    ...
    async def _get_station_stream_details(self, item_id: str) -> StreamDetails:
        ...
        if not station:
            raise MusicAssistantError("Couldn't get stream details for station %s", item_id)

        if not station.stream:
            raise MusicAssistantError("No stream found for %s", item_id)
