...
# Regex to validate provider IDs (alphanumeric, dashes, underscores, dots)
...
class MusicMeProvider(MusicProvider):
    ...
    async def _browse_home(self) -> Sequence[MediaItemType | ItemMapping | BrowseFolder]:
        """Browse the MusicMe homepage ('A l'affiche')."""
    ...
    async def _browse_radio_themes(
        ...
        tid = theme_id or "0"
        data = await self._api_get(
            f"/radios?filters={{theme:{tid}}}"
            f"&resources=themes,theme-airplays{{maxResults:50}},home"
        )
        if not data:
            return []
        results: list[MediaItemType | ItemMapping | BrowseFolder] = []
    ...
    # ---- recommendations (powers the Discover page) ----
    ...
    async def get_library_radios(self) -> AsyncGenerator[Radio, None]:
        ...
        data = await self._api_get(
            "/radios?filters={theme:0}"
            "&resources=themes,theme-airplays{maxResults:100},home"
        )
        if not data:
            return
        for item in data.get("results", {}).get("theme-airplays", []):
    ...
    async def get_radio(self, prov_radio_id: str) -> Radio:
        """Get a single radio by id, loading full details from the airplay endpoint."""
    ...
    async def get_track(self, prov_track_id: str) -> Track:
        ...
        """
        album_barcode = prov_track_id.split("-")[0]
    ...
    async def get_artist_toptracks(self, prov_artist_id: str) -> list[Track]:
        ...
        """
        data = await self._api_get(
            f"/artist/{prov_artist_id}?resources=albums{{maxResults:5}}"
        )
        if not data:
            return []
    ...
    async def get_artist_albums(self, prov_artist_id: str) -> list[Album]:
        ...
        data = await self._api_get(
            f"/artist/{prov_artist_id}?resources=albums{{maxResults:50}}"
        )
        if not data:
            return []
        albums = data.get("results", {}).get("albums", [])
    ...
    async def _get_radio_track(self, radio_id: str) -> str:
        ...
        :param radio_id: The radio airplay ID (e.g. 'rd-1163935' or 'art-6726').
        """
    ...
    # ---- parsers: MusicMe JSON → MA models ----
    ...
    def _parse_artist(self, artist_obj: dict[str, Any]) -> Artist:
        ...
        # Use square artist images (r288) instead of the default
        # banner-shaped eyestoeye/144x56 which display poorly as thumbnails
        if artist_id:
    ...
    def _parse_track(self, track_obj: dict[str, Any]) -> Track:
        ...
        if track_obj.get("album"):
            track.album = self._parse_album(
                {"barcode": barcode.split("-")[0], "name": track_obj["album"]}
            )
    ...
    async def _web_search_fallback(
        ...
        The web search (search.php) redirects to an artist/album page if there's a match.
        We then extract the artist ID from window.playerInit and load their data via the API.
        ...
        if not content_id or not _VALID_ID_RE.match(str(content_id)):
            return None
    ...
    # ---- private helpers: auth & API ----

    async def _login(self) -> None:
        """Authenticate with MusicMe via web login and extract the userId.

        The flow is:
        1. POST email/password to the web login form -> session cookies
        2. GET the logged-in frameset page -> parse window.playerInit JSON for userId
        """
    ...
    async def _api_get(self, endpoint: str) -> dict[str, Any] | None:
        ...
        try:
            return json_loads(_decrypt(raw.strip()))
    ...
    async def _get_stream_url(self, track_barcode: str) -> str:
        ...
        :param track_barcode: The track barcode (e.g. '5034644330297-01_01').
        """
        ...
        # The ticket is double-encrypted: the API response was already decrypted,
        # but the ticket value itself needs a second decryption pass
        ticket = _decrypt(data["ticket"])
