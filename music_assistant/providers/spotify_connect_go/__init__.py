...
class SpotifyConnectGoProvider(PluginProvider):
    ...
    async def handle_async_init(self) -> None:
        ...
        if not os.path.exists(self._go_librespot_bin):
            raise FileNotFoundError(f"go-librespot binary not found at {self._go_librespot_bin}")
    ...
    async def _websocket_listener(self) -> None:
        ...
        while not self._stop_called and retry_count < max_retries:
            try:
                ...
                retry_count += 1
                self.logger.warning(
                    "WebSocket connection failed (attempt %d/%d): %s", retry_count, max_retries, e
                )
        ...
        if event_type in ("metadata", "track_changed", "new_track"):
            ...
            if player:
                ...
                player_attrs = [
                    ...
                    if "next" in attr.lower() or "prev" in attr.lower() or "skip" in attr.lower()
                ]
                self.logger.info("Player attributes related to skip: %s", player_attrs)

                self.logger.info(
                    "Player active_source: %s, our instance_id: %s",
                    player.active_source,
                    self.instance_id,
                )
                ...
                self.logger.debug(
                    "Player %s updated with source %s", self.mass_player_id, self.instance_id
                )
            ...
            if self._source_details.in_use_by:
                ...
                if player:
                    ...
                    if hasattr(player, "_attr_elapsed_time_last_updated"):
                        player._attr_elapsed_time_last_updated = time.time()
                        self.logger.debug(
                            "Froze elapsed_time at %s seconds", player._attr_elapsed_time
                        )
                ...
                if "position" in data:
                    ...
                    if self._source_details.metadata:
                        ...
                        if old_position > position_sec + 3:
                            self.logger.info(
                                "Position jumped backwards from %s to %s - track restarted",
                                old_position,
                                position_sec,
                            )
                    ...
                    if self._source_details.in_use_by:
                        ...
                        if player:
                            ...
                            if (
                                ...
                                and self._source_details.metadata.duration
                            ):
                                position_sec = min(
                            ...
                            if hasattr(player, "_attr_elapsed_time"):
                                ...
                                player._attr_elapsed_time_last_updated = time.time()
                                self.logger.debug(
                                    "Set player position to %s at timestamp %s",
                                    position_sec,
                                    time.time(),
                                )
    ...
    async def _update_metadata(self, metadata: dict) -> None:
        ...
        self.logger.debug(
            "_update_metadata called with metadata keys: %s",
            ...
            if player:
                ...
                if hasattr(player, "_attr_elapsed_time"):
                    ...
                    if media.duration and safe_elapsed_time > media.duration:
                        ...
                        self.logger.warning(
                            "Capping elapsed_time from %s to duration %s",
                            media.elapsed_time,
                            media.duration,
                        )
                    ...
                    player._attr_elapsed_time_last_updated = time.time()
                    self.logger.debug(
                        "Set player elapsed_time to %s at %s",
                        safe_elapsed_time,
