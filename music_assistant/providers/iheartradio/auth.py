...
class IHeartRadioSession:
    """A session with iHeartRadio, for a signed-in account or a guest."""
...
class IHeartRadioAuthManager:
    """Hold the provider's session with iHeartRadio, a guest session when no account is set."""

    def __init__(self, provider: IHeartRadioProvider) -> None:
        """Initialize the auth manager."""
        self.provider = provider
        self.logger = provider.logger
        self.session: IHeartRadioSession | None = None
    ...
    async def relogin(self, stale: IHeartRadioSession | None = None) -> None:
        """
        Open a new session, signed in when credentials are configured.

        :param stale: The session the caller found rejected; when another caller has
            already replaced it, nothing is done.
        """
        async with self._lock:
            if stale is not None and self.session is not stale:
                return
            username = self._username()
        ...
        if username:
            self.logger.info("Signed in to iHeartRadio with an account")
        else:
            self.logger.info("Signed in to iHeartRadio as a guest (no account configured)")
    ...
    async def _account_login(self, username: str, password: str) -> dict[str, Any]:
        ...
        payload = await self.provider.api.request(
            ...
            form={
                ...
                "host": self.provider.api.host_name,
            },
            authenticated=False,
        )
    ...
    def _stored_session(self) -> IHeartRadioSession | None:
        ...
        session_id = self.provider.get_setup_value(CONF_SESSION_ID)
        if not profile_id or not session_id:
            return None
        username = self.provider.get_setup_value(CONF_SESSION_USERNAME)
        return IHeartRadioSession(str(profile_id), str(session_id), str(username or ""))
    ...
    def _persist(self, session: IHeartRadioSession) -> None:
        ...
        """
        self.provider._update_setup_data(CONF_PROFILE_ID, session.profile_id)
        self.provider._update_setup_data(CONF_SESSION_ID, session.session_id)
        self.provider._update_setup_data(CONF_SESSION_USERNAME, session.username)
