...
class IHeartRadioStreamingManager:
    ...
    async def _station_stream(self, item_id: str) -> StreamDetails:
        ...
        """
        station = await self.provider.api.get_station(item_id)
        if station is None:
            raise MediaNotFoundError(f"Station {item_id} not found")
        # the station url comes from the cached station details; it redirects to a
        # per-listener stream with a short-lived ad token, and that target is never kept
    ...
    async def _episode_stream(self, item_id: str) -> StreamDetails:
        ...
        episode = await self.provider.api.get_episode(episode_id)
        if episode is None:
