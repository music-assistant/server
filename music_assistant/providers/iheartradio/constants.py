...
# Preference order for a station's stream variants. HLS is what iHeart's own web player
...
# Analytics context sent on station and playback calls. The API accepts any of its known
# values, this one is what the website sends from an artist page.
...
# How long a batch's track urls are served for. The urls carry no expiry; a probe found them
# still served after four hours. A track from an older batch is refused rather than handed
# to ffmpeg, so a station paused longer than this has to be started again.
BATCH_URL_TTL: Final[int] = 4 * 3600
