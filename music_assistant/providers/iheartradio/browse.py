...
class IHeartRadioBrowseManager:
    ...
    def _root_folders(self, path: str) -> list[BrowseFolder]:
        """Return the top level folders for live radio and podcasts."""
    ...
    async def _browse_markets(self, path: str) -> list[BrowseFolder]:
        ...
        for market in await self.provider.api.get_markets():
    ...
    async def _browse_genres(self, path: str) -> list[BrowseFolder]:
        ...
        base = _base_path(path)
        folders: list[BrowseFolder] = []
        for genre in await self.provider.api.get_genres():
    ...
    async def _browse_categories(self, path: str) -> list[BrowseFolder]:
        ...
        base = _base_path(path)
        folders: list[BrowseFolder] = []
        for category in await self.provider.api.get_podcast_categories():
    ...
    async def _browse_category(self, category_id: str) -> list[MediaItemType]:
        ...
        return [
            podcast
            for item in await self.provider.api.get_podcast_category(category_id)
    ...
    async def _stations(
        ...
        return [
            station
            for item in await self.provider.api.get_stations(market_id=market_id, genre_id=genre_id)
