...
def parse_live_station(station: Mapping[str, Any], instance_id: str, domain: str) -> Radio | None:
    ...
    if not (station_id := _as_id(station.get("id"))):
        return None
    website = _website_url(station.get("website"))
    radio = Radio(
        item_id=station_id,
...
def parse_track(
    ...
    mass_track = Track(
        ...
        # the catalog calls the disc number volume
        disc_number=int(track.get("volume") or 0),
        provider_mappings={
            ProviderMapping(
                item_id=track_id,
                provider_domain=domain,
...
def parse_album(album: Mapping[str, Any], instance_id: str, domain: str) -> Album | None:
    ...
    if (release_date := _as_epoch_seconds(album.get("releaseDate"))) is not None:
        mass_album.year = from_utc_timestamp(release_date).year
...
def parse_podcast_episode(
    ...
    if (start_date := _as_epoch_seconds(episode.get("startDate"))) is not None:
        mass_episode.metadata.release_date = from_utc_timestamp(start_date)
...
def _website_url(value: Any) -> str | None:
    """Return a station website as a full url, or None when there is none."""
    if not value:
        return None
    website = str(value)
    # most stations list their site without a scheme
    return website if "://" in website else f"https://{website}"


def _as_id(value: Any) -> str | None:
    """Return an id as string, or None when it is missing."""
