...
class IHeartRadioLibraryManager:
    ...
    async def get_library_radios(self) -> AsyncGenerator[Radio]:
        ...
        for entry in await self._follow_entries(PATH_FOLLOWS_LIVE):
            if not (station_id := entry.get("liveStationId")):
                continue
            try:
                station = await self.provider.api.get_station(str(station_id))
    ...
    async def library_remove(self, prov_item_id: str, media_type: MediaType) -> bool:
        ...
        if media_type == MediaType.PODCAST:
            ...
            return False
        await self.provider.api.request("DELETE", path)
        return True
...
def _as_int(value: str) -> int | str:
    """Return an id as the number the follow endpoints expect, or unchanged when not numeric."""
