...
FireRed's `max(speech, singing)` probabilities are averaged at 100ms resolution, then resampled to 1800 fixed bins spanning the track duration for `AudioAnalysisData.vocal_activity`. FireRedVAD source and AED model weights are Apache-2.0 licensed; attribution is recorded in the project `NOTICE`.
