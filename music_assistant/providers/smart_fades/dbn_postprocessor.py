...
Reimplements madmom's DBNDownBeatTrackingProcessor without the Cython
dependency. Uses a bar-pointer HMM with Viterbi decoding to enforce
tempo-continuous beat tracking.

Based on:
- Böck et al., "A Multi-Model Approach to Beat Tracking" (ISMIR 2014)
...
def _viterbi(
    ...
    for t in range(num_frames):
        ...
        if len(multi_states) > 0:
            ...
            cur_v[multi_states] = scores[np.arange(len(multi_states)), best_idx] + obs[multi_states]
            bt[t] = best_idx
...
class DBNDownBeatTracker:
    ...
    def __init__(
        ...
        for num_beats in beats_per_bar:
            ...
            om_pointers = _build_observation_model(positions, observation_lambda)
            self._hmms.append(
                {
                    "num_beats": num_beats,
                    "positions": positions,
    ...
    def __call__(self, activations: NDArray[np.float64]) -> NDArray[np.float64]:
        ...
        log_dens = _compute_log_densities(activations, self.observation_lambda)
