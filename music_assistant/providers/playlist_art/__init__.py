"""Playlist Art Metadata Provider for Music Assistant.

Generates custom artwork for library playlists using artist images,
replacing the default album-grid collage with artist-focused layouts.
...
from music_assistant_models.media_items import Artist, ItemMapping, MediaItemImage, Playlist, Track
...
async def setup(
    ...
    return PlaylistArtProvider(mass, manifest, config, SUPPORTED_FEATURES)
...
async def get_config_entries(
    ...
    return (
        ConfigEntry(
            ...
            description=(
                "Choose how playlist artwork is generated. "
                "'Album Grid' uses album covers in a grid layout. "
                "'Artist Mosaic' places the most-played artist large in the centre "
                "with others as smaller tiles. "
                "'Artist Grid' shows up to four unique artist images in equal tiles. "
                "'Artist Radio' centers the main artist on a solid background with overlapping circles. "
                "'Artist Banner' overlays the playlist name on a full-bleed artist image (Tidal / Essentials style). "
                "'Album Fan' stacks album covers as framed photo cards, slightly rotated (Apple Music style). "
                "'Album Grid Tilted' renders a grid with white gaps and rotates the whole image ~15° (Apple Music tilted collage style)."
            ),
            required=True,
            default_value=TEMPLATE_ALBUM_GRID,
            options=[
                ConfigValueOption(title="Album Grid", value=TEMPLATE_ALBUM_GRID),
                ConfigValueOption(title="Album Fan", value=TEMPLATE_ALBUM_FAN),
                ConfigValueOption(title="Album Grid Tilted", value=TEMPLATE_ALBUM_GRID_TILTED),
                ConfigValueOption(title="Artist Mosaic", value=TEMPLATE_ARTIST_MOSAIC),
                ConfigValueOption(title="Artist Grid", value=TEMPLATE_ARTIST_GRID),
                ConfigValueOption(title="Artist Radio", value=TEMPLATE_ARTIST_RADIO),
                ConfigValueOption(title="Artist Banner", value=TEMPLATE_ARTIST_BANNER),
            ],
        ...
        ConfigEntry(
            ...
            label="Skip provider playlists",
            description=(
                "When enabled, provider playlists (e.g. from Spotify or Tidal) that "
                "already have their own artwork are skipped. Playlists without any "
                "artwork still receive a generated collage."
            ),
            required=False,
            default_value=True,
...
class PlaylistArtProvider(MetadataProvider):
    """Metadata provider that generates artist-based artwork for library playlists."""
    ...
    def _is_our_image(self, img: MediaItemImage) -> bool:
        ...
        Handles both absolute paths (written by current code) and bare basenames (written by
        older versions), because the frontend's Edit Playlist dialog can also overwrite the
        provider field to "builtin".
        """
    ...
    async def generate_playlist_artwork(
        self, playlist: Playlist, force_refresh: bool = False
    ) -> MediaItemImage | None:
        """Generate artwork image for the given playlist without updating the database.

        Called by the metadata controller during its regular refresh cycle.
        Returns a MediaItemImage (written to local storage) or None if generation was skipped.

        :param playlist: The playlist to generate artwork for.
        :param force_refresh: If True, use unique filename for cache busting.
        """
        ...
        if skip_provider:
            ...
            if not has_builtin and not has_smart_playlist and playlist.metadata.images:
                return None
        return await self._generate_and_write(playlist, unique_filename=force_refresh)
    ...
    async def _generate_and_write(  # noqa: PLR0915
        ...
        """Render artwork and persist it to disk.

        :param playlist: The playlist to generate artwork for.
        :param template: Optional template override; None uses the plugin's configured default.
        :param unique_filename: When True, include a timestamp in the filename so the browser
            does not serve a cached version of the old image (used by manual regeneration).
        """
        ...
        # Stable filename so repeated refreshes produce the same path (deduplication).
        # unique_filename=True appends a timestamp for browser cache-busting on manual regen.
        # Absolute path: the frontend hardcodes provider="builtin" on save, so resolve_image()
        # is bypassed and builtin falls back to os.path.isfile() on the absolute path.
        if unique_filename:
            filename = f"{playlist.item_id}_{int(time())}_thumb.jpg"
        else:
            filename = f"{playlist.item_id}_thumb.jpg"
        file_path = os.path.join(self._images_dir, filename)
        # Use mkstemp for a unique temp path to avoid writer collisions if two calls
        # regenerate the same playlist concurrently (e.g. batch refresh vs update_metadata).
        ...
        # Note: Thumbnail cache invalidation happens automatically via file modification time
