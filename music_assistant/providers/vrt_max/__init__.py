...
requires authentication and is added in a later phase.
"""
...
async def setup(
    ...
    return VrtMaxProvider(mass, manifest, config, SUPPORTED_FEATURES)
    ...
    async def browse(self, path: str) -> Sequence[MediaItemType | ItemMapping | BrowseFolder]:
        ...
        if subpath == BROWSE_RADIO_PROGRAMS:
            return await self._browse_landing(
                RADIO_LANDING_PAGE, _RADIO_ROW_TYPE, BROWSE_RADIO_PROGRAMS
            )
        if subpath == BROWSE_PODCASTS:
            return await self._browse_landing(
                PODCAST_LANDING_PAGE, _PODCAST_ROW_TYPE, BROWSE_PODCASTS
            )
