"""Shared base class for filesystem-backed music providers."""
...
class FileSystemProvider(MusicProvider):
    """
    Base class shared by every filesystem-backed music provider.
    ...
    Supports m3u files for playlists. Subclasses override the low-level I/O primitives
    (``resolve``, ``exists``, ``_scandir``, ``_read_file``, ...) to talk to a mounted
    local path, a remote share, or a cloud/HTTP API.
    """

    # parallel workers per sync; subclasses lower this for slower transports
    _SYNC_CONCURRENCY: ClassVar[int] = 16
    # safe default for any subclass; LocalFileSystemProvider (and SMB/NFS, which reuse
    # its local-path I/O after mounting) override this to True since scanning a
    # mounted/local disk is cheap
    _background_analysis_default_enabled: ClassVar[bool] = False
    ...
    async def get_config_entries(self) -> tuple[ConfigEntry, ...]:
        ...
        return (
            ...
            dataclasses.replace(
                CONF_ENTRY_BACKGROUND_ANALYSIS_ENABLED,
                default_value=self._background_analysis_default_enabled,
            ),
        )
