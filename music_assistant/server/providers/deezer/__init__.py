...
class DeezerProvider(MusicProvider):
    """Implementation of a deezer MusicProvider."""
    ...
    async def get_library_artists(self) -> AsyncGenerator[Artist, None]:
        """Retrieve library artists from deezer."""
        endpoint = "me/following"
        while True:
            deezer_artists = await self._get_data(
                endpoint,
                type="artist",
                limit=50,
            )
            for item in deezer_artists["artists"]["items"]:
                if item and item["id"]:
                    yield await self._parse_artist(item)
            if deezer_artists["artists"]["next"]:
                endpoint = deezer_artists["artists"]["next"]
                endpoint = endpoint.replace("https://api.deezer.com/", "")
            else:
                break
    ...
    async def add_playlist_tracks(self, prov_playlist_id: str, prov_track_ids: list[str]):
        ...
        for track_id in prov_track_ids:
            track_uris.append(f"deezer:track:{track_id}")
    ...
    async def remove_playlist_tracks(
        ...
        for track in await self.get_playlist_tracks(prov_playlist_id):
            if track.position in positions_to_remove:
                track_uris.append({"uri": f"deezer:track:{track.item_id}"})
    ...
    async def get_audio_stream(
        ...
        args = [
            ...
            "320",
            "--single-track",
            f"deezer://track:{streamdetails.item_id}",
        ]
        if seek_position:
    ...
    async def _parse_artist(self, artist_obj):
        """Parse deezer artist object to generic layout."""
        ...
        artist.add_provider_mapping(
            ProviderMapping(
                ...
                url=artist_obj["external_urls"]["deezer"],
            )
        )
    ...
    async def _parse_album(self, album_obj: dict):
        """Parse deezer album object to generic layout."""
        ...
        album.add_provider_mapping(
            ProviderMapping(
                ...
                content_type=ContentType.OGG,
                bit_rate=320,
                url=album_obj["external_urls"]["deezer"],
            )
        )
        return album

    async def _parse_track(self, track_obj, artist=None):
        """Parse deezer track object to generic layout."""
        ...
        track.add_provider_mapping(
            ProviderMapping(
                ...
                content_type=ContentType.OGG,
                bit_rate=320,
                url=track_obj["external_urls"]["deezer"],
    ...
    async def _parse_playlist(self, playlist_obj):
        """Parse deezer playlist object to generic layout."""
        playlist = Playlist(
        ...
        playlist.add_provider_mapping(
            ProviderMapping(
                ...
                provider_domain=self.domain,
                provider_instance=self.instance_id,
                url=playlist_obj["external_urls"]["deezer"],
            )
        )
    ...
    async def login(self) -> dict:
        """Log-in deezer and return tokeninfo."""
        ...
        if tokeninfo and userinfo:
            ...
            self.logger.info("Successfully logged in to deezer as %s", userinfo["id"])
            self._auth_token = tokeninfo
            return tokeninfo
        if tokeninfo and not userinfo:
            raise LoginFailed(
                "Unable to retrieve userdetails from deezer API - probably just a temporary error"
            )
        if self.config.get_value(CONF_USERNAME).isnumeric():
            # a deezer free/basic account can be recognized when
            # the username consists of numbers only - check that here
            # an integer can be parsed of the username, this is a free account
            raise LoginFailed("Only deezer Premium accounts are supported")
        raise LoginFailed(f"Login failed for user {self.config.get_value(CONF_USERNAME)}")

    async def _get_token(self):
        """Get deezer auth token with librespot bin."""
        ...
        try:
            ...
            self.logger.warning(
                "Error while retrieving deezer token after %s seconds, details: %s",
                duration,
                stdout.decode(),
            )
            return None
        self.logger.debug(
            "Retrieved deezer token using librespot in %s seconds",
            duration,
        )
