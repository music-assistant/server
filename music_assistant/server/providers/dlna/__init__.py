...
class DLNAPlayer:
    ...
    def update_attributes(self):
        ...
        if self.available:
            ...
            self.player.current_item_id = self.device._get_current_track_meta_data(
                "queue_item_id"
            )
        else:
...
class DLNAPlayerProvider(PlayerProvider):
    ...
    async def cmd_play_media(
        ...
        # await dlna_player.device.async_set_transport_uri(url, queue_item.name, metadata)
        # Play it
    ...
    async def poll_player(self, player_id: str) -> None:
        ...
        try:
            ...
            await self._update_player(dlna_player)
        except UpnpError as err:
    ...
    async def _enqueue_next_track(
        ...
        didl_metadata = _create_didl_metadata(url, next_item, is_radio)
        try:
            ...
            self.logger.info("Player does not suport next uri")
    ...
    async def _update_player(self, dlna_player: DLNAPlayer) -> None:
        ...
        if dlna_player.player.state == PlayerState.PLAYING and (
            ...
            self.mass.create_task(
                self._enqueue_next_track(dlna_player, current_item_id)
            )
        ...
        # ):
        #     # player does not support next uri, manual play it
        #     await self.mass.players.queues.play_index(
        #         dlna_player.udn, dlna_player.next_item
        #     )
...
def _create_didl_metadata(url: str, queue_item: QueueItem, radio: bool = False) -> str:
    ...
    return (
        ...
        f"<upnp:channelName>{queue_item.queue_item_id}</upnp:channelName>"
