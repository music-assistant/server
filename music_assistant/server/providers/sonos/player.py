...
class SonosPlayer:
        ...
        async with self._subscription_lock:
            try:
                ...
                if not subscriptions:
                    return
                self.logger.verbose("Creating subscriptions for %s", self.zone_name)
    ...
    async def unsubscribe(self) -> None:
        ...
        if not self._subscriptions:
            return
        self.logger.verbose("Unsubscribing from events for %s", self.zone_name)
    ...
    async def check_poll(self) -> None:
        ...
        if not self.should_poll:
            return
        self.logger.verbose("Polling player for availability...")
        try:
    ...
    def _speaker_activity(self, source: str) -> None:
        ...
        self.logger.verbose("Activity on %s from %s", self.zone_name, source)
