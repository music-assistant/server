...
# TODO: port code to Databases
# TODO: Remove controller specific code
...
class DatabaseController:
    ...
    def __init__(self, mass: MusicAssistant, url: DatabaseURL):
        ...
        mass.subscribe(self.on_shutdown_event, EventType.SHUTDOWN)
    ...
    async def on_shutdown_event(self, event: EventType, details: EventDetails) -> None:
    ...
    async def get_playlists(
        ...
        async with aiosqlite.connect(self._dburl, timeout=360) as db_conn:
    ...
    async def fetchall(
        ...
        """Return (mapped) data for given query."""
        if params is None:
            params = tuple()
        async with aiosqlite.connect(self._dburl, timeout=360) as db_conn:
            db_conn.row_factory = aiosqlite.Row
            return await db_conn.execute_fetchall(sql_query, params)

    async def fetchone(
        self, sql_query: str, params: Tuple[Any] | None = None
    ) -> Mapping | None:
        """Return (mapped) data for given query."""
        for item in self.fetchall(sql_query, params):
            return item
        return None

    async def execute(self, sql_query: str, params: Tuple[Any] | None = None) -> int:
        """Perform action on db. return last row id."""
        if params is None:
            params = tuple()
        async with aiosqlite.connect(self._dburl, timeout=360) as db_conn:
            async with db_conn.execute(sql_query, params) as cursor:
                last_row_id = cursor.lastrowid
            await db_conn.commit()
            return last_row_id

    async def get_rows(self, table: str) -> Iterable[Mapping]:
        """Get all rows for given table."""
        return await self.fetchall(f"SELECT * FROM {table}")

    async def get_row(self, table: str, key: str, value: str) -> Mapping | None:
        """Get single row for given table where column matches key/value."""
        return await self.fetchone(f"SELECT * FROM {table} WHERE ? = ?", (key, value))

    async def insert_or_replace(self, table: str, data: Dict[str, Any]) -> int:
        """Insert or replace data in given table."""
        keys = data.keys()
        values = tuple(data[x] for x in keys)
        sql_query = f"""INSERT OR REPLACE INTO {table}
                ({','.join(keys)})
                VALUES({'?,'*len(keys)});"""
        return await self.execute(sql_query, values)
