    ...
    async def _register_loaded_provider(
        ...
        async def _on_provider_loaded() -> None:
            try:
                ...
                # the provider stays registered and available even when a post load step
                # failed, so signal it either way: an event left unset makes every waiter
                # pay the full timeout, on every attempt, until the provider reloads
