...
class PlaybackTrackerMixin(_PlayerQueuesBase):
    ...
    def _update_queue_from_player(
        ...
        if "current_item_id" in changed_keys:
            ...
            elif queue.autoplay_enabled and data.enqueued_media_items and running_low:
    ...
    def _get_flow_queue_stream_index(
        ...
        """
        data = self._queue_data[queue.queue_id]
        elapsed_time_queue_total = player.state.corrected_elapsed_time or 0
        if queue.current_index is None and not data.flow_mode_stream_log:
        ...
        track_time = 0.0
        for play_log_entry in data.flow_mode_stream_log:
    ...
    def _handle_end_of_queue(
        ...
        async def _clear_or_resume_delayed() -> None:
            ...
            if dynamic_playlist is None:
                dynamic_playlist = next(
                    (
                        item
                        for item in reversed(data.enqueued_media_items)
                        if isinstance(item, Playlist) and item.is_dynamic
                    ),
                    None,
                )
            if dynamic_playlist is not None:
                try:
                    ...
                    playback_user = (
                        await self.mass.webserver.auth.get_user(data.userid)
                        if data.userid
                        else None
                    )
        ...
        if queue.flow_mode and data.flow_mode_stream_log:
            last_log_entry = data.flow_mode_stream_log[-1]
        ...
        if self._should_mark_played(
            ...
            self.mass.create_task(
                self.mass.music.mark_item_played(
                    ...
                    userid=data.userid,
                    queue_id=queue.queue_id,
                    user_initiated=self._is_user_initiated_play(data, media_item),
            ...
            if fully_played and not is_playing:
                if credit_album := self._enqueued_album_for_track(data, item_to_report, media_item):
                    self.mass.create_task(self._mark_album_played(credit_album, media_item, data))
    ...
    def _enqueued_album_for_track(
        self, data: PlayerQueueData, item_to_report: QueueItem, media_item: MediaItemType
    ) -> Album | None:
        """
        ...
        enqueued = next(
            (
                item
                for item in data.enqueued_media_items
    ...
    def _is_user_initiated_play(self, data: PlayerQueueData, media_item: MediaItemType) -> bool:
        """Return whether a played item was explicitly chosen by the user."""
        return media_item in data.enqueued_media_items

    async def _mark_album_played(
        self, album: Album, track: MediaItemType, data: PlayerQueueData
    ) -> None:
