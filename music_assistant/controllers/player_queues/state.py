...
class PlayerQueueData:
    ...
    # when the user last switched shuffle on for this queue. Deliberately not persisted: a shuffle
    # restored from cache belongs to an earlier listening session and must not be read as intent
    # for whatever the user plays next (see PlayerQueuesController._apply_shuffle_intent).
