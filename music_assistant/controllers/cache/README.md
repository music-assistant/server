...
- Entries with `use_expired_cache=True` survive the daily auto-cleanup task even after they have expired, so they remain available as fallback data for the stale-while-revalidate path of `@use_cache`. This is independent of `persistent`: `persistent` controls explicit `clear()` calls, `use_expired_cache` controls auto-cleanup of expired rows.
- The `@use_cache` decorator accepts `use_expired_cache=True` to enable stale-while-revalidate: an expired entry is returned immediately and a background refresh updates the cache for the next request. The `BYPASS_CACHE` context variable still forces a synchronous re-fetch.
...
- A daily cleanup task removes expired entries (unless `use_expired_cache=True`). Oversized databases (>2GB) are removed entirely on startup.
