...
class MetaDataController(CoreController):
    ...
    async def _update_playlist_metadata(
        ...
        new_images = await self._generate_playlist_artwork(
            playlist, all_playlist_tracks_images, cur_images, force_refresh
        )
        # IMPORTANT: Replace images completely, don't merge with old ones
    ...
    async def _generate_playlist_artwork(
        self,
        playlist: Playlist,
        all_playlist_tracks_images: list[MediaItemImage],
        cur_images: list[MediaItemImage],
        force_refresh: bool = False,
    ) -> list[MediaItemImage]:
        """Generate artwork for a playlist using provider or fallback to collage."""
        # Start with a fresh list - we'll explicitly add what we want to keep
        result_images = []

        # Look for the playlist_art metadata provider
        playlist_art_provider = None
        for provider in self.mass.providers:
            if provider.domain == "playlist_art":
                playlist_art_provider = provider
                break

        # Generate artwork using the provider if available
        generated_by_provider = False
        if playlist_art_provider:
            try:
                if artwork_image := await playlist_art_provider.generate_playlist_artwork(  # type: ignore[union-attr]
                    playlist, force_refresh=force_refresh
                ):
                    result_images.append(artwork_image)
                    generated_by_provider = True
                    self.logger.debug(
                        "Generated playlist artwork using %s provider", playlist_art_provider.name
                    )
            except Exception as err:
                self.logger.warning(
                    "Failed to generate playlist artwork using %s: %s",
                    playlist_art_provider.name,
                    err,
                    exc_info=err if self.logger.isEnabledFor(logging.DEBUG) else None,
                )

        # Fallback to collage images if no artwork was generated by provider
        if not generated_by_provider:
            # thumb image
            ...
            if not thumb_image or self._collage_images_dir in thumb_image.path:
                ...
                if collage_thumb_image := await self.create_collage_image(
                    ...
                    result_images.append(collage_thumb_image)
            elif thumb_image:
                # just use old image
                result_images.append(thumb_image)
            # fanart image
            ...
            if not fanart_image or self._collage_images_dir in fanart_image.path:
                ...
                if collage_fanart_image := await self.create_collage_image(
                    ...
                    result_images.append(collage_fanart_image)
            elif fanart_image:
                # just use old image
                result_images.append(fanart_image)
        else:
            # Provider generated artwork - keep non-THUMB images from current (e.g. fanart)
