...
class FeatureAccumulator:
    ...
    def add_low_freq_rms(self, low_freq_rms: npt.NDArray[np.float32]) -> None:
        """Add low-frequency RMS energy chunk.

        :param low_freq_rms: Low-freq RMS values for this chunk, shape (1, n_frames).
        """
        with self._lock:
    ...
    def get_low_freq_rms(self) -> npt.NDArray[np.float32]:
        """Get concatenated low-frequency RMS energy.

        :return: Full low-freq RMS array, shape (1, total_frames).
        """
        with self._lock:
            if not self.low_freq_rms_chunks:
                return np.zeros((1, 0), dtype=np.float32)
