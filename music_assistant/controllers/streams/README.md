...
- **Reachability probe**: `GET /info` answers with the server id and a permissive CORS header, so a browser on the local network can check that the published address leads to this server. The `streams/info` API command reports that address (base URL, published IP and port).
