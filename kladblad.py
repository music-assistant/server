...
class deezerProvider(MusicProvider):
    ...
    async def search(
        ...
        if len(media_types) == 1:
            # deezer does not support multiple searchtypes, falls back to all if no type given
    ...
    async def get_library_tracks(self) -> AsyncGenerator[Track, None]:
        """Retrieve library tracks from deezer."""
        endpoint = "favorite/getUserFavorites"
    ...
    async def get_similar_artists(self, prov_artist_id):
        ...
        # https://www.deezer.com/api.json/0.2/artist/getSimilarArtists?artist_id=220020&offset=0&limit=3
    ...
    async def _report_playback_started(self, streamdata: dict) -> None:
        """Report playback start to deezer."""
        # TODO: need to figure out if the streamed track is purchased by user
        # https://www.deezer.com/api.json/0.2/purchase/getUserPurchasesIds?limit=5000&user_id=xxxxxxx
    ...
    async def _report_playback_stopped(self, streamdetails: StreamDetails) -> None:
        """Report playback stop to deezer."""
        user_id = self._user_auth_info["user"]["id"]
    ...
    async def _parse_artist(self, artist_obj: dict):
        ...
        artist.add_provider_mapping(
            ProviderMapping(
                ...
                url=artist_obj.get("url", f'https://open.deezer.com/artist/{artist_obj["id"]}'),
            )
        )
    ...
    async def _parse_album(self, album_obj: dict, artist_obj: dict = None):
        ...
        album.add_provider_mapping(
            ProviderMapping(
                ...
                url=album_obj.get("url", f'https://open.deezer.com/album/{album_obj["id"]}'),
            )
        )
    ...
    async def _parse_track(self, track_obj: dict):
        ...
        track.add_provider_mapping(
            ProviderMapping(
                ...
                url=track_obj.get("url", f'https://open.deezer.com/track/{track_obj["id"]}'),
            )
        )
        return track
        ...
        playlist.add_provider_mapping(
            ProviderMapping(
                ...
                url=playlist_obj.get(
                    "url", f'https://open.deezer.com/playlist/{playlist_obj["id"]}'
                ),
            )
        )
    ...
    async def _auth_token(self):
        """Login to deezer and store the token."""
        ...
        if details and "user" in details:
            ...
            self.logger.info(
                "Successfully logged in to deezer as %s", details["user"]["display_name"]
            )
    ...
    async def _get_data(self, endpoint, sign_request=False, **kwargs):
        """Get data from api."""
        # pylint: disable=too-many-branches
        url = f"http://www.deezer.com/api.json/0.2/{endpoint}"
    ...
    def __get_image(self, obj: dict) -> str | None:
        """Try to parse image from deezer media object."""
        if obj.get("image"):
