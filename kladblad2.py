...
class PlexProvider(MusicProvider):
    ...
    async def setup(self) -> None:
        ...
        self._plex_library = await self._run_async(
            self._plex_server.library.section, self.config.get_value(LIBRARY_NAME)
        )
    ...
    async def _parse_playlist(self, plex_playlist: PlexPlaylist) -> Playlist:
        ...
        if plex_playlist.thumbUrl:
            playlist.metadata.images = [
                MediaItemImage(ImageType.THUMB, plex_playlist.thumbUrl, True)
            ]
    ...
    async def _parse_track(self, plex_track: PlexTrack) -> Track:
        ...
        track.artist = await self._parse_artist(await self._run_async(plex_track.artist))
    ...
    async def get_library_tracks(self) -> AsyncGenerator[Track, None]:
        ...
        tracks_obj = await self._search_track(None, limit=9999)
