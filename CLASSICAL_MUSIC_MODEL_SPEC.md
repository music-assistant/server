...
17. **Grouping recordings on Work detail.** *Resolved:* by album context, with movements from the same album collapsed under one recording entry. Uses the performance grouping heuristic (Work + conductor + ensemble + album) from Matching policy.
18. **Sort order on Work detail recordings.** *Resolved:* recording year descending by default (newest first). User can toggle to conductor / ensemble sort.
19. **What entity powers OTHER VERSIONS on Work detail.** *Resolved:* reuse the existing OTHER VERSIONS component populated with unmatched Work suggestions from the same composer.
...
21. **Context menu navigation entry for classical entities.** *Resolved:* the pop-music pattern of *omitting* the navigation entry when multiple artists are credited deliberately does not apply in the Classical view — classical tracks routinely have many role-typed credits and users navigate through them by design.
