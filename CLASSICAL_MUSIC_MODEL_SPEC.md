...
24. **Handling missing `MOVEMENTNUMBER` when `MOVEMENTNAME` is present.** *Resolved as intent for the Stage 4 parser:* when a track has `MOVEMENTNAME` but no explicit `MOVEMENTNUMBER`, the parser fills the gap in the following order.
    ...
    3. **Range sanity check.** If the inferred number exceeds `MOVEMENTTOTAL` (when set), discard it as spurious (the leading numeral is probably part of the title rather than a movement index) and fall through.
    4. **File-order fallback.** For tracks still missing a number, sort the tracks of the same `(album, work)` group by file position on disk and assign sequential integers, filling gaps left by tracks that do have `MOVEMENTNUMBER` set explicitly.
    5. **Give up.** If none of the above resolves, leave `movement_number` null. Better a hole than a fabricated wrong number.

    Movement number is deliberately **not** inferred from `TRACKNUMBER` or `DISCNUMBER` — a track's position on its source album can bear no relation to its movement position on the underlying Work (compilations, split-disc boundaries, prelude tracks, filler tracks around a work). The `(album, work)` scope on file-order fallback is what stops that mismatch.
