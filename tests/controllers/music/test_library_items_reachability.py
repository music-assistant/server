...
def _mapping(
    provider_instance: str, provider_domain: str, item_id: str, *, in_library: bool = True
) -> ProviderMapping:
...
async def test_reachable_via_includes_item_not_in_library_on_requested_provider(
    ...
    `provider` filter.
    """
    monkeypatch.setattr(mass.music, "get_unique_providers", lambda: ["local_1", "spotify_1"])
    await _add_track(
        mass,
        "Track A",
        {
...
async def test_reachable_via_excludes_item_for_provider_outside_user_access(
    ...
    """
    A provider outside the current user's access (per get_unique_providers) is ignored.

    `get_unique_providers` already applies the current user's provider_filter, so a
...
async def test_reachable_via_none_applies_no_filter(
    ...
    monkeypatch.setattr(mass.music, "get_unique_providers", list)
