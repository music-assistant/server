...
class FakeDynamicRadioProvider(MusicProvider):
    ...
    toggle_station_is_endless_stream: bool = True
    ...
    async def get_library_radios(self) -> AsyncGenerator[Radio]:
        ...
        yield Radio(
            ...
            is_endless_stream=self.toggle_station_is_endless_stream,
    ...
    async def get_radio(self, prov_radio_id: str) -> Radio:
        ...
        is_endless_stream = prov_radio_id != FINITE_STATION_ID
        name = (
            "Dynamic Station"
            if is_dynamic
            else "Finite Station"
...
class TestVersionsDynamicGuard:
    ...
    async def test_finite_station_has_no_other_versions(
        ...
        provider.toggle_station_is_endless_stream = False
...
class TestSyncLibraryRadiosDynamicFlag:
    ...
    async def test_flipping_endless_stream_updates_library_item(
        ...
        library_item = await radio_ctrl.get_library_item_by_prov_mappings([self._toggle_mapping()])
        assert library_item is not None
        assert library_item.is_endless_stream is True
        ...
        library_item = await radio_ctrl.get_library_item_by_prov_mappings([self._toggle_mapping()])
        assert library_item is not None
        assert library_item.is_endless_stream is False
