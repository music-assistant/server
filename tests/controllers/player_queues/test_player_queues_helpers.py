...
def _radio(*, is_dynamic: bool, is_endless_stream: bool = True, name: str = "R") -> Radio:
    return Radio(
        item_id=name.lower(),
        provider="test",
        name=name,
...
class TestIsDynamicSource:
    ...
    def test_finite_radio(self) -> None:
        ...
        assert is_dynamic_source(_radio(is_dynamic=False, is_endless_stream=False)) is False

    def test_stream_radio(self) -> None:
        """A live-stream radio does not feed the pool."""
        assert is_dynamic_source(_radio(is_dynamic=False, is_endless_stream=True)) is False
