...
async def test_resolve_finite_radio_expands_to_its_tracks_and_marks_played() -> None:
    """A finite dynamic radio (an AI Radio show) resolves to its whole tracklist, once."""
    resolver, mark = _resolver()
    radio = Radio(
        item_id="r1",
        provider="library",
    ...
    track = Track(item_id="t1", provider="library", name="T1", provider_mappings=set())
    resolver.mass.music.radio.dynamic_tracks = AsyncMock(  # type: ignore[method-assign]
        return_value=[track]
    )
    ...
    assert result == [track]
    resolver.mass.music.radio.dynamic_tracks.assert_awaited_once_with(radio)
    assert mark.call_args.kwargs["user_initiated"] is True
