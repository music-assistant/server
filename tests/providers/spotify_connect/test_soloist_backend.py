...
class _FakeProc:
    ...
    def __init__(self, exit_code: int = 0, start_error: Exception | None = None) -> None:
...
async def test_daemon_argv_and_key_never_logged(
    ...
    backend._stop_called = True  # single supervisor iteration
    spawned = _patch_spawn(monkeypatch, [_FakeProc()])
...
async def test_exit_code_10_refreshes_binary_before_restart(
    ...
    class _FakeManager:
        ...
        async def ensure_fresh(self, consent: bool) -> Path:
            """Serve the replacement build and end the test after this restart."""
            refreshed.append(consent)
            backend._stop_called = True
            return Path("/fake/bin/soloist-v2")

    monkeypatch.setattr(soloist_backend, "SoloistBinaryManager", _FakeManager)
    spawned = _patch_spawn(monkeypatch, [_FakeProc(exit_code=10), _FakeProc(exit_code=0)])
...
def test_audio_formats_are_capture_pcm() -> None:
    """Both audio formats advertise the fixed capture PCM (source quality unknown)."""
    backend, _events = _make_backend()
    ...
    assert backend.decoded_audio_format == fmt
