...
def _make_session(
    *,
    ...
    async def _default_finish(_session: SetupSession, submitted: dict[str, Any]) -> dict[str, str]:
    ...
    return SetupSession(mass, "flow-test", context, finish or _default_finish), collected
...
async def test_new_setup_go_librespot_path() -> None:
    """The go-librespot branch finishes right after the backend choice."""
...
async def test_new_setup_soloist_path() -> None:
    """The soloist branch collects consent and the API key, then finishes."""
    session, collected = _make_session()
...
async def test_reconfigure_soloist_keeps_stored_key_on_empty_input() -> None:
    ...
    with mock.patch.object(spotify_flow, "verify_platform_supported"):
        ...
        session.handle_submit({CONF_API_KEY: ""})
        await _wait_finished(session)
        await task
...
async def test_switch_soloist_to_go_librespot_clears_secrets_on_finish() -> None:
    """Switching to go-librespot wipes the soloist secrets when finish succeeds."""
    session, collected = _make_session(kind="reconfigure", setup_data=dict(_SOLOIST_SETUP_DATA))
    task, _step = await _start_flow(session)
...
async def test_cancel_before_finish_persists_nothing() -> None:
    """Cancelling the flow before finish leaves the stored soloist secrets alone."""
    session, collected = _make_session(kind="reconfigure", setup_data=dict(_SOLOIST_SETUP_DATA))
    task, _step = await _start_flow(session)
...
async def test_switch_go_librespot_to_soloist_collects_soloist_values() -> None:
    """Switching to soloist runs the consent and API key steps, then finishes."""
    session, collected = _make_session(
        kind="reconfigure", setup_data={CONF_BACKEND: BACKEND_GO_LIBRESPOT}
    )
    with mock.patch.object(spotify_flow, "verify_platform_supported"):
        task, _step = await _start_flow(session)
        step = await _submit(session, {CONF_BACKEND: BACKEND_SOLOIST})
...
async def test_finish_failure_returns_to_backend_choice() -> None:
    """A failed finish re-renders the backend choice with the error; a retry succeeds."""
    collected: dict[str, Any] = {}
    failures = iter([True, False])

    async def finish(_session: SetupSession, submitted: dict[str, Any]) -> dict[str, str]:
        if next(failures):
            raise SetupFlowError("boom")
        collected.update(submitted)
        return {"instance_id": "spotify_connect--test"}

    session, _unused = _make_session(finish=finish)
    task, _step = await _start_flow(session)

    step = await _submit(session, {CONF_BACKEND: BACKEND_GO_LIBRESPOT})
    assert step.step_id == "backend"
    assert step.errors == {"base": "boom"}
