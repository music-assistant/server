...
from music_assistant.providers.iheartradio.api import IHeartRadioApiClient
...
from music_assistant.providers.iheartradio.stations import IHeartRadioStationManager
...
def provider(api: FakeApi) -> IHeartRadioProvider:
    ...
    provider.api = IHeartRadioApiClient(provider, "au")
    provider.stations = IHeartRadioStationManager(provider)
    ...
    provider.api.request = api.request  # type: ignore[method-assign]
    provider.api.get_json = api.get_json  # type: ignore[method-assign]
    ...
    # nothing configured or persisted yet
    provider.get_setup_value = Mock(return_value=None)  # type: ignore[method-assign]
    provider._update_setup_data = Mock()  # type: ignore[method-assign]
