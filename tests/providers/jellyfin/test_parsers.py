...
import aiohttp
import pytest
from aiojellyfin import Album, Artist, Connection, Track
...
ALBUM_DECODER = JSONDecoder(Album)
TRACK_DECODER = JSONDecoder(Track)
...
async def test_parse_artists(
    ...
    connection: Connection,  # pylint: disable=redefined-outer-name
    snapshot: SnapshotAssertion,
) -> None:
    """Test we can parse artists."""
    async with aiofiles.open(example) as fp:
        raw_data = ARTIST_DECODER.decode(await fp.read())
    parsed: dict[str, Any] = parse_artist(
        _LOGGER, "xx-instance-id-xx", connection, raw_data
    ).to_dict()
    # sort external Ids to ensure they are always in the same order for snapshot testing
    parsed["external_ids"].sort()  # pylint: disable=unsubscriptable-object
...
async def test_parse_albums(
    ...
    async with aiofiles.open(example) as fp:
        raw_data = ALBUM_DECODER.decode(await fp.read())
    parsed: dict[str, Any] = parse_album(
        _LOGGER, "xx-instance-id-xx", connection, raw_data
    ).to_dict()
    # sort external Ids to ensure they are always in the same order for snapshot testing
    parsed["external_ids"].sort()  # pylint: disable=unsubscriptable-object
...
async def test_parse_tracks(
    ...
    async with aiofiles.open(example) as fp:
        raw_data = TRACK_DECODER.decode(await fp.read())
    parsed: dict[str, Any] = parse_track(
        _LOGGER, "xx-instance-id-xx", connection, raw_data
    ).to_dict()
    # sort external Ids to ensure they are always in the same order for snapshot testing
    if (
        parsed.get("external_ids")  # pylint: disable=unsubscriptable-object
    ):
        parsed["external_ids"].sort()  # pylint: disable=unsubscriptable-object
    assert snapshot == parsed
