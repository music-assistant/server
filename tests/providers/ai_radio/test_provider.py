...
async def test_unload_on_removal_prunes_the_mirrored_library_rows() -> None:
    """Removing the plugin removes its mirrored radio rows; a plain unload keeps them."""
    provider, _, _ = _make_engine_provider([])
    remover = AsyncMock()
    provider._remove_show_library_items = remover  # type: ignore[method-assign]
