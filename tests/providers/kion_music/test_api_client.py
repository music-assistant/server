"""Unit tests for KionMusicClient (api_client.py)."""
...
def _make_client() -> tuple[KionMusicClient, mock.AsyncMock]:
    """Create a KionMusicClient with a mocked underlying ClientAsync.

    :return: Tuple of (KionMusicClient, mock_underlying_client).
    """
    client = KionMusicClient(token="fake_token")
...
async def test_get_my_mix_tracks_returns_tracks_and_batch_id() -> None:
    """get_my_mix_tracks calls rotor_station_tracks and returns ordered tracks and batch_id."""
    client, underlying = _make_client()
    ...
    full_track = type("Track", (), {"id": 100, "title": "My Mix Track"})()
    underlying.tracks = mock.AsyncMock(return_value=[full_track])

    tracks, batch_id = await client.get_my_mix_tracks()
...
async def test_get_my_mix_tracks_empty_sequence_returns_empty() -> None:
    """When rotor returns no sequence, get_my_mix_tracks returns ([], batch_id or None)."""
    client, underlying = _make_client()
