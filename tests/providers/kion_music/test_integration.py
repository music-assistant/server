...
from music_assistant.providers.kion_music.constants import BROWSE_NAMES_EN, BROWSE_NAMES_RU
...
def _load_yandex_objects() -> tuple[Any, Any, Any, Any]:
    """Load Kion Artist, Album, Track, Playlist from fixtures for mock client."""
...
async def test_search(mass: MusicAssistant) -> None:
    ...
    yandex_tracks = [t for t in results.tracks if t.provider and "kion_music" in t.provider]
...
async def test_get_playlist_tracks_page_gt_zero_returns_empty(mass: MusicAssistant) -> None:
    """Page > 0 returns empty list (Kion returns all tracks in one call)."""
    prov = _get_yandex_provider(mass)
    assert prov is not None
