...
async def test_library_artist_parser_failure_reports_unidentifiable_item(
    ...
    with (
        mock.patch.object(provider, "report_skipped_sync_item") as report_skipped,
        mock.patch("music_assistant.providers.kion_music.provider.parse_artist", side_effect=error),
    ):
        assert [item async for item in provider.get_library_artists()] == []

    report_skipped.assert_called_once_with(MediaType.ARTIST, None, error)
...
async def test_library_album_parser_failure_reports_album_id(
    ...
    with (
        mock.patch.object(provider, "report_skipped_sync_item") as report_skipped,
        mock.patch("music_assistant.providers.kion_music.provider.parse_album", side_effect=error),
    ):
        assert [item async for item in provider.get_library_albums()] == []

    report_skipped.assert_called_once_with(MediaType.ALBUM, "42", error)
...
async def test_library_track_parser_failure_reports_track_id(
    ...
    with (
        mock.patch.object(provider, "report_skipped_sync_item") as report_skipped,
        mock.patch("music_assistant.providers.kion_music.provider.parse_track", side_effect=error),
    ):
        assert [item async for item in provider.get_library_tracks()] == []

    report_skipped.assert_called_once_with(MediaType.TRACK, "51", error)
...
async def test_library_playlists_parser_failure_reports_owner_and_kind(
    ...
    with (
        ...
        mock.patch.object(
            provider,
            "get_playlist",
            new=mock.AsyncMock(side_effect=[mock.sentinel.wave, mock.sentinel.liked]),
        ),
        mock.patch(
    ...
    report_skipped.assert_called_once_with(MediaType.PLAYLIST, "77:88", error)
...
async def test_library_tracks_reports_partial_and_empty_batches(
    ...
    assert report_skipped.call_count == 2
    first_call, second_call = report_skipped.call_args_list
...
async def test_liked_tracks_playlist_parser_failure_only_logs_locally(
    ...
    report_skipped.assert_not_called()
    cast("mock.MagicMock", provider.logger).debug.assert_any_call(
        "Error parsing liked track %s: %s", "61:7", error
    )
