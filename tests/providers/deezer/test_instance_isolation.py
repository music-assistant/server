...
from music_assistant.providers.deezer.gw_client import GW_LIGHT_URL, MEDIA_GET_URL, GWClient
...
async def test_arl_is_sent_per_request() -> None:
    ...
    assert not client.session.cookie_jar
...
@pytest.mark.parametrize("url", [GW_LIGHT_URL, MEDIA_GET_URL])
async def test_cookies_are_filtered_for_the_request_url(url: str) -> None:
    """Neutralize cookies for the target endpoint without changing the shared jar."""
    session = _session(unrelated="www-only")
    session.cookie_jar.update_cookies({"account": "other-account"}, URL(url))
    client = GWClient(session, "arl-one")

    cookies = client._request_cookies(url)

    assert cookies["account"] == ""
    assert ("unrelated" in cookies) == (url == GW_LIGHT_URL)
    assert session.cookie_jar.filter_cookies(URL(url))["account"].value == "other-account"
