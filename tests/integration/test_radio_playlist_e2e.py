...
@pytest.mark.asyncio
async def test_get_dynamic_radio_tracks_honors_track_filter(e2e_mass: MusicAssistant) -> None:
    ...
    baseline = await e2e_mass.music.get_dynamic_radio_tracks([seed], include_base_tracks=True)
    ...
    with track_filter(lambda track: track.item_id != excluded) as active:
        filtered = await e2e_mass.music.get_dynamic_radio_tracks([seed], include_base_tracks=True)
