...
with nothing queued there is nothing to play, seek or skip through.
"""
...
def test_filled_queue_advertises_transport_controls(
    ...
    mock_mass.player_queues.get = MagicMock(return_value=MagicMock(items=5))
...
def test_empty_queue_reports_transport_controls_unavailable(
    ...
    mock_mass.player_queues.get = MagicMock(return_value=MagicMock(items=0))
